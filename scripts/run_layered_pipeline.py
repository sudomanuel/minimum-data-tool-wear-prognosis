...
from phm.dataset_builder import get_feature_columns, read_dataset_meta
...
def step_cleanup() -> tuple:
    ...
    keep_in_root = {
        ...
        "output_manifest.csv",
        # analysis artifacts (NOT benchmark outputs) — must not be archived
        "physics_validation.csv", "feature_redundancy_vif.csv",
        "feature_correlation.csv", "ingest_inventory.csv",
        "c1_lambda_mono_sweep_SOLO_A_N_ST.csv",
        "c2_lambda_rate_sweep_FUSION_N_ST.csv",
        # P1 closure artifacts (nested tuning before/after)
        "benchmark_before_after_nested_tuning_minimal.csv",
        "nested_tuning_audit.csv", "p1_closure_summary.csv",
        # P2 time-aware baselines
        "time_aware_results.csv", "time_aware_vs_sensor_only.csv",
        # P3 PINN comparison
        "pinn_comparison_results.csv", "pinn_fold_predictions.csv",
        # P4 RUL derivation
        "rul_derivation_results.csv", "rul_threshold_summary.csv",
        # P5 uncertainty
        "uncertainty_vb_bands.csv", "uncertainty_failure_distribution.csv",
        "uncertainty_rul_distribution.csv", "structural_uncertainty_summary.csv",
    }
...
def main():
    ...
    _meta = read_dataset_meta(PROCESSED_DATASET)
    if _meta:
        print(f"[INFO] dataset provenance: builder={_meta.get('builder')} "
              f"schema={_meta.get('schema')} n_features={_meta.get('n_features')} "
              f"created={_meta.get('created')}")
