...
Tuning (NESTED — corregido, Punto 1 del roadmap):
  - El tuning se ejecuta DENTRO de cada fold LOEO, usando GroupKFold solo
    sobre los 9 experimentos de entrenamiento de ese fold.
  - El experimento held-out NUNCA participa en la seleccion de
    hiperparametros: cada fold elige sus propios params de forma
    independiente y se refittean sobre su propio train.
  - Esto es nested-CV honesto: elimina el sesgo optimista de seleccion que
    tenia el esquema previo (tuning unico sobre los 10 experimentos).
  - Augmentation: el tuning (inner CV) usa SOLO los experimentos REALES del
    train; las filas sinteticas se aplican unicamente al FIT final de cada
    fold. Asi el inner-validation fold nunca contiene datos augmentados ni
    sinteticos derivados del mismo experimento que se valida.
  - El modelo "full-data" que se entrena al final (para SHAP) se tunea por
    separado sobre los datos REALES (fit sobre los datos de la rama); es una
    explicacion post-hoc, NO forma parte de la evaluacion LOEO.
"""
...
from .modeling import (
    ...
    build_svr, build_rf, build_xgb, build_mlp, build_bnn, XGBOOST_AVAILABLE,
)
...
def all_baseline_builders() -> Dict[str, callable]:
    ...
    out['MLP'] = build_mlp
    # BNN (metodo del paper, baseline de comparacion). build_bnn() devuelve
    # None si torch no esta; el harness ya omite builders que devuelven None.
    if build_bnn() is not None:
        out['BNN'] = build_bnn
    return out
...
def _tune_one(name: str, pipe, X_all, y_all, groups_all,
    ...
    """
    Tune `pipe` con metodo ∈ {'random', 'grid'} usando GroupKFold sobre los
    datos que reciba (X_all, y_all, groups_all). Devuelve
    (best_params, best_cv_mae, info). Si el modelo no es tuneable, dict vacio.

    Funcion generica: el caller decide el alcance. En el flujo nested
    (run_branch) se invoca con SOLO los 9 experimentos de train de cada fold
    LOEO, de modo que el experimento held-out nunca informa la seleccion de
    hiperparametros. Tambien se invoca una vez sobre todos los datos para el
    modelo full-data usado en SHAP (explicacion post-hoc, fuera de LOEO).
    """
...
def run_branch(branch_id: str,
    ...
    # Modelo full-data (SOLO para SHAP; explicacion post-hoc, fuera de LOEO):
    #   - se TUNEA sobre datos REALES (full_df, sin sinteticos), para no sesgar
    #     la seleccion de hiperparametros con augmentation;
    #   - se ENTRENA sobre los datos de la rama (augmentados si A).
    tune_df = full_df
    if data_branch == 'A':
        tune_df, _, _ = _augment_or_keep(full_df, aug_strategy, seed)
    X_tune = tune_df[feat_cols].values.astype(float)            # fit (aug si A)
    y_tune = tune_df[TARGET_COLUMN].values.astype(float)
    X_tune_real = full_df[feat_cols].values.astype(float)       # tuning (real)
    y_tune_real = full_df[TARGET_COLUMN].values.astype(float)
    groups_tune_real = full_df[EXPERIMENT_ID_COL].astype(int).values
    ...
    # ---------- Step 1: LOEO-CV con tuning NESTED (dentro de cada fold) ------
    # El tuning ya NO se hace una sola vez sobre los 10 experimentos. Ahora
    # cada fold LOEO tunea usando SOLO sus 9 experimentos de train (GroupKFold
    # interno), de modo que el experimento held-out nunca informa la seleccion
    # de hiperparametros. Elimina el sesgo optimista del esquema anterior.
    ...
    # Acumuladores de tuning por fold (uno por modelo): registro auditable.
    fold_params: Dict[str, list] = {name: [] for name in builders}
    fold_cv_mae: Dict[str, list] = {name: [] for name in builders}
    fold_tune_info: Dict[str, list] = {name: [] for name in builders}
    ...
    for tr_df_fold, te_df_fold in loeo_iter(full_df, group_col=EXPERIMENT_ID_COL):
        ...
        # Datos para el FIT final del fold: augmentados si la rama es A.
        X_tr = tr_aug[feat_cols].values.astype(float)
        y_tr = tr_aug[TARGET_COLUMN].values.astype(float)
        # Datos para el TUNING (inner CV): SOLO los 9 experimentos REALES del
        # train, sin filas sinteticas. Garantiza que el inner-validation fold
        # nunca contiene datos augmentados (Punto 1, item 6 del roadmap). La
        # augmentation es estrategia de FIT, no de seleccion de modelo.
        X_tr_real = tr_df_fold[feat_cols].values.astype(float)
        y_tr_real = tr_df_fold[TARGET_COLUMN].values.astype(float)
        groups_tr_real = tr_df_fold[EXPERIMENT_ID_COL].astype(int).values
        ...
        for name, builder in builders.items():
            ...
            try:
                if tuning_method == 'none' or name not in TUNABLE_MODELS:
                    ...
                    # Tuning NESTED sobre datos REALES del train (9 experimentos,
                    # sin sinteticos). Asserts anti-leakage: (1) el held-out no
                    # entra a las groups; (2) el tuning no ve filas augmentadas
                    # (n_filas == n_experimentos reales del train).
                    assert held_out.isdisjoint(set(groups_tr_real.tolist())), (
                        f"[{branch_id}/fold{fold_idx}/{name}] held-out {held_out} "
                        f"presente en groups de tuning — LEAKAGE")
                    fp, fcv, finfo = _tune_one(
                        name, base_pipe, X_tr_real, y_tr_real, groups_tr_real,
                        tuning_method
                    )
                    fold_params[name].append(fp)
                    fold_cv_mae[name].append(fcv)
                    fold_tune_info[name].append(finfo)
                    est = _apply_params(base_pipe, fp)
                with warnings.catch_warnings():
                    warnings.simplefilter('ignore')
                    est.fit(X_tr, y_tr)
    ...
    # ---------- Step 2: agregacion + entrenamiento final para SHAP ----------
    for name, builder in builders.items():
        ...
        # Entrenar estimador final sobre dataset completo para SHAP.
        # Este modelo es una explicacion post-hoc del modelo entrenado en TODO
        # el dataset; NO forma parte de la evaluacion LOEO. Por eso aqui SI se
        # tunea sobre todos los datos (separado de las metricas LOEO de arriba).
        base_pipe = builder()
        if base_pipe is None:
            continue
        try:
            if tuning_method == 'none' or name not in TUNABLE_MODELS:
                ...
                best_params_per_model[name] = {}
            else:
                full_params, _, _ = _tune_one(
                    name, base_pipe, X_tune_real, y_tune_real, groups_tune_real,
                    tuning_method
                )
                best_params_per_model[name] = full_params
                est_full = _apply_params(base_pipe, full_params)
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
            ...
            best_params_per_model.setdefault(name, {})

        # ---- Registro de tuning NESTED para auditoria (uno por modelo) ----
        tunable = (tuning_method != 'none' and name in TUNABLE_MODELS)
        valid_cv = [c for c in fold_cv_mae[name] if np.isfinite(c)]
        params_keys = {
            json.dumps({k: _safe(v) for k, v in fp.items()}, sort_keys=True)
            for fp in fold_params[name]
        }
        tuning_rows.append({
            'model': name,
            'feature_subset': feature_subset,
            ...
            'branch_id': branch_id,
            'n_features': len(feat_cols),
            # compat: 'best_params' ahora lista los params elegidos por fold.
            'best_params': json.dumps(
                [{k: _safe(v) for k, v in fp.items()} for fp in fold_params[name]]
                if tunable else []),
            # compat: media del MAE de CV interno across folds.
            'best_cv_score_mae': float(np.mean(valid_cv)) if valid_cv else float('nan'),
            'best_params_full_data': json.dumps(
                {k: _safe(v) for k, v in best_params_per_model.get(name, {}).items()}),
            'n_folds_tuned': len(fold_params[name]) if tunable else 0,
            'params_stable_across_folds': (len(params_keys) <= 1)
                                          if (tunable and fold_params[name]) else None,
            'scoring': 'neg_mean_absolute_error',
            'cv_strategy': ('NESTED: GroupKFold(k=5) sobre los 9 experimentos de '
                            'train de cada fold LOEO (held-out excluido)'),
            'notes': json.dumps({'method': tuning_method,
                                 'per_fold_info': fold_tune_info[name]}
                                if tunable else {'method': 'none'}),
        })
