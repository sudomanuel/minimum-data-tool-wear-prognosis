...
# =============================================================================
# 00 — Diagrama de flujo del PROYECTO (herramientas + modelos, end-to-end)
# =============================================================================
def plot_project_flow_diagram(target_dir: Path,
                              best_model_name: str = None,
                              best_mae: float = None,
                              filename: str = '00_project_flow_diagram') -> Path:
    """Flujo end-to-end del proyecto con las herramientas actuales.

    Datos crudos → build_dataset → experiment_features.csv → dos tracks:
      BENCHMARK : run_layered_pipeline (36 ramas LOEO) → modelos → ranking + SHAP
      FÍSICA·PINN: validate_physics → PINN VB=f(x,t) (autodiff) → run_pinn (LOEO)
    Ambos convergen en el reporte; nota de futuro: multi-cuchilla → LOTO.

    Todo queda enmarcado (frame exterior) y dimensionado para 16:9.
    """
    NAVY, STEEL, RUST = '#1F3A5F', '#5B7A9C', '#D9742B'
    INKC, GRAY = '#1F2937', '#9AA3AF'
    PAL = {
        ...
        'future': dict(fc='#F3F4F6', ec=GRAY,  tc='#4B5563'),
    }

    fig, ax = plt.subplots(figsize=(16, 9))
    ax.set_xlim(0, 100); ax.set_ylim(0, 100); ax.axis('off')

    # Frame exterior (todo encuadrado)
    ax.add_patch(FancyBboxPatch(
        (2.5, 3.0), 95, 92, boxstyle="round,pad=0.2,rounding_size=1.4",
        linewidth=2.2, edgecolor=NAVY, facecolor='none', zorder=1))

    def _box(cx, cy, w, h, text, key, fs=10, weight='bold', dashed=False):
        p = PAL[key]
        ax.add_patch(FancyBboxPatch(
            (cx - w / 2, cy - h / 2), w, h,
        ...
        ax.text(cx, cy, text, ha='center', va='center', fontsize=fs,
                color=p['tc'], weight=weight, zorder=4)
        return dict(top=(cx, cy + h / 2), bottom=(cx, cy - h / 2), cx=cx, cy=cy)

    def _arrow(a, b, color=STEEL, lw=2.0, ls='-'):
        ax.add_patch(FancyArrowPatch(
            a, b, arrowstyle='-|>,head_length=9,head_width=6',
            linewidth=lw, color=color, shrinkA=1, shrinkB=3, zorder=2,
            linestyle=ls))

    def _line(a, b, color=GRAY, lw=2.0):
        ax.plot([a[0], b[0]], [a[1], b[1]], color=color, lw=lw,
                solid_capstyle='round', zorder=1)

    # Titulo + pill de run.py
    ax.text(50, 98, 'Flujo del proyecto — datos, herramientas y modelos',
            ha='center', va='center', fontsize=18, weight='bold', color=INKC)
    ax.add_patch(FancyBboxPatch(
        (27, 89.3), 46, 3.6, boxstyle="round,pad=0.2,rounding_size=1.8",
        linewidth=1.4, edgecolor=NAVY, facecolor='#EEF2F8', zorder=3))
    ax.text(50, 91.1, 'python run.py   ·   dataset  →  benchmark | pinn  →  report',
            ha='center', va='center', fontsize=11.5, weight='bold',
            color=NAVY, zorder=4)

    # Spine compartido
    RAW = _box(50, 83, 54, 6,
               'Senales de vibracion crudas\n'
               '{canal}{cuchilla}_{exp}_p{parte}   ·   axial · rotacional · acustica',
               'data', fs=10.5)
    BUILD = _box(50, 73, 28, 5, 'build_dataset.py', 'data', fs=11.5)
    FEATS = _box(50, 63, 46, 6,
                 'experiment_features.csv\n'
                 '1 fila por experimento   ·   features de ancho fijo',
                 'data', fs=10.5)
    _arrow(RAW['bottom'], BUILD['top'], color=NAVY)
    _arrow(BUILD['bottom'], FEATS['top'], color=NAVY)

    # Split a dos tracks
    bus_y = 57.0
    _line((50, FEATS['bottom'][1]), (50, bus_y), color=NAVY)
    _line((27, bus_y), (73, bus_y), color=NAVY)
    ax.text(27, 59.3, 'BENCHMARK', ha='center', fontsize=12.5,
            weight='bold', color=STEEL)
    ax.text(73, 59.3, 'FISICA · PINN', ha='center', fontsize=12.5,
            weight='bold', color=RUST)

    # Track izquierdo: benchmark
    LP = _box(27, 51, 36, 6,
              'run_layered_pipeline.py\n36 ramas   ·   LOEO honesto', 'bench', fs=10.5)
    MODELS = _box(27, 40, 36, 8,
                  'Modelos\nDummy · Ridge · Lasso · ElasticNet\n'
                  'SVR · RandomForest · XGBoost · MLP · BNN', 'bench', fs=9.0)
    RANK = _box(27, 30, 32, 5, 'Ranking   +   SHAP', 'bench', fs=11)
    _arrow((27, bus_y), LP['top'], color=STEEL)
    _arrow(LP['bottom'], MODELS['top'], color=STEEL)
    _arrow(MODELS['bottom'], RANK['top'], color=STEEL)

    # Track derecho: fisica / PINN
    PHYS = _box(73, 51, 38, 6,
                'validate_physics_assumptions.py\n'
                'monotonia rho=1.0 · tasa<->E_rot rho=0.76', 'phys', fs=9.3)
    PINN = _box(73, 40, 38, 8,
                'PINN    VB = f(x, t)\n'
                'df/dt >= 0      ·      dVB/dt = g(E_rot)\n'
                '(determinista · autodiff)', 'phys', fs=9.5)
    RUNP = _box(73, 30, 34, 5, 'run_pinn.py — LOEO + busqueda', 'phys', fs=10.5)
    _arrow((73, bus_y), PHYS['top'], color=RUST)
    _arrow(PHYS['bottom'], PINN['top'], color=RUST)
    _arrow(PINN['bottom'], RUNP['top'], color=RUST)

    # Convergencia → reporte
    conv_y = 24.5
    _line((27, RANK['bottom'][1]), (27, conv_y), color=STEEL)
    _line((73, RUNP['bottom'][1]), (73, conv_y), color=RUST)
    _line((27, conv_y), (73, conv_y), color=GRAY, lw=1.8)
    REPORT = _box(50, 19, 44, 6,
                  'Reporte (PPTX)\nbuild_pipeline_presentation   ·   _en',
                  'report', fs=11)
    _arrow((50, conv_y), REPORT['top'], color=NAVY)

    # Futuro
    FUT = _box(50, 9, 58, 5,
               'Proximo:  ingesta multi-cuchilla  ->  Leave-One-Tool-Out (LOTO)',
               'future', fs=10.5, dashed=True)
    _arrow(REPORT['bottom'], FUT['top'], color=GRAY, ls='--', lw=1.6)

    # Nota del mejor modelo (si se pasa)
    if best_model_name and best_mae is not None and np.isfinite(best_mae):
        ax.text(50, 1.6,
                f'Mejor LOEO actual:  {best_model_name}  ·  MAE = {best_mae:.2f} um',
