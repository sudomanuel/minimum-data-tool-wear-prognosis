...
from .config import (
    ...
    NON_FEATURE_COLS, PROCESS_METADATA_COLS, PROCESSED_DATASET, PROCESSED_DIR,
    CONTACT_FEATURES, MODELING_DATASET, FEATURE_COLUMNS_CSV, INTERIM_DIR,
    METRICS_DIR, DATASET_META_SUFFIX, DATASET_SCHEMA_LEGACY,
)
...
def build_dataset(segments_dir: Path = SEGMENTS_DIR,
    ...
    write_dataset_meta(PROCESSED_DATASET, "build_dataset", DATASET_SCHEMA_LEGACY, dataset)
    if verbose:
...
# Utilidades
# -----------------------------------------------------------------------------
def get_feature_columns(df: pd.DataFrame, drop_constant: bool = True) -> list:
    """Columnas numericas que son features ML legitimas (C3).

    Excluye: ids/target/flags (NON_FEATURE_COLS), proceso/metadata
    (PROCESS_METADATA_COLS, p.ej. VC/F), columnas no numericas y — si
    drop_constant — columnas casi-constantes (std < 1e-9: ruido muerto como
    `mean` tras centrar, o duration/n_samples constantes). La deteccion de
    constantes necesita >=3 filas; con menos no se descarta nada.
    """
    do_const = drop_constant and len(df) >= 3
    out, dropped = [], []
    for c in df.columns:
        if c in NON_FEATURE_COLS or c in PROCESS_METADATA_COLS:
            continue
        ...
        out.append(c)
    if dropped:
        warnings.warn(f"[FEATURES] {len(dropped)} columnas casi-constantes excluidas "
                      f"(ej: {dropped[:4]})")
    return out


def write_dataset_meta(out_path, builder: str, schema: str,
                       df: pd.DataFrame) -> dict:
    """Sidecar JSON con la procedencia del dataset (C1)."""
    meta = {
        "builder": builder,
        "schema": schema,
        "n_rows": int(len(df)),
        "n_features": int(len(get_feature_columns(df))),
        "n_tools": int(df[TOOL_ID_COL].nunique()) if TOOL_ID_COL in df.columns else None,
        "created": time.strftime("%Y-%m-%d %H:%M:%S"),
    }
    p = Path(str(out_path) + DATASET_META_SUFFIX)
    try:
        p.write_text(json.dumps(meta, indent=2), encoding="utf-8")
    except Exception as exc:
        warnings.warn(f"[META] no se pudo escribir {p.name}: {exc}")
    return meta


def read_dataset_meta(dataset_path) -> Optional[dict]:
    """Lee el sidecar de procedencia si existe (C1)."""
    p = Path(str(dataset_path) + DATASET_META_SUFFIX)
    if not p.exists():
        return None
    try:
